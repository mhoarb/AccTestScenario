...
func generateReport(results []AttemptResult) {
	...
	for _, result := range results {
		...
		if result.StatusCode == 400 && len(result.Errors) > 0 && result.Errors[0] == "اطلاعات وارد شده صحیح نمی باشد." {
			invalidCredsCount++
		}
	...
	fmt.Printf("Invalid UserName and correct pass: %d\n", invalidCredsCount)
	fmt.Printf("Invalid pass and correct username %d\n", len(results)-successCount-rateLimitCount-invalidCredsCount)

	fmt.Println("\nDetailed Results:")
	for _, result := range results {
		fmt.Printf("Attempt %d: Status Code = %d, Cif = %s ,Password: %s , Timestamp = %s, Status = %s, Message = %s, Errors = %v, Success = %v\n",
			result.AttemptNumber, result.StatusCode, result.UsedCif, result.USerPassword, result.Timestamp, result.Status, result.Message, result.Errors, result.IsSuccess)
	}
	fmt.Println("========================")
}
